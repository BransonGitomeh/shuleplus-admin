...
class ResultsMatrix extends React.Component {
  ...
  handleBulkSend = async (template) => {
      const { assessments, selectedTerm } = this.state;
      const students = this.getFilteredStudents();
      // Safe access to Data.terms
      const currentTerm = (this.state.terms || []).find(t => t.id === selectedTerm);
      ...
      for (const student of students) {
          if (!student.parent?.phone) continue;
           ...
           const msg = template
                .replace(/\[Name\]/g, student.names.split(' ')[0])
                .replace(/\[Mean\]/g, mean)
    ...
    if (showPrintView) {
        return (
            <div className="bg-white">
                ...
                <style>{`
                    @media print {
                        .d-print-none, .kt-header, .kt-aside, .kt-footer { display: none !important; }
                        body, .kt-content { background: white !important; padding: 0 !important; margin: 0 !important; }
                        #kt_wrapper { padding: 0 !important; }
