...
var Data = (function () {
    ...
    const entityConfigs = [
        ...
        { name: "students", singularName: "student", createFields: ['names', 'route', 'gender', 'registration', 'parent', 'school', 'parent2', 'class', 'paidFees'], updateFields: ['names', 'route', 'registration', 'gender', 'parent', 'parent2', 'class', 'paidFees'], customMethods: (allData, subs) => ({ getPage: async ({ page = 1, limit = 15 }) => { const offset = (page - 1) * limit; const response = await query(`query GetStudentPage($limit: Int, $offset: Int, $id: String) { school(id: $id) { studentsCount students(limit: $limit, offset: $offset) { id names gender registration class{id, name} route{id, name} parent{id, name} paidFees feeStatus { balance balanceFormated } } } }`, { limit, offset, id: localStorage.getItem("school") }); const processedStudents = response.school?.students?.map(s => ({ ...s, parent_name: s.parent?.name, class_name: s.class?.name })) || []; return { students: processedStudents, totalCount: response.school?.studentsCount || 0 }; } }) },
        {
            name: "parents",
            singularName: "parent",
